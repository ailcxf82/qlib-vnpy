                ...
                try:
                    ...
                    for inst in instruments:
                        try:
                            ...
                            # 处理 MultiIndex 的情况（qlib 返回的通常是 MultiIndex）
                            if isinstance(price_data.index, pd.MultiIndex):
                                # 使用 xs 方法提取单个股票的数据
                                try:
                                    stock_data = price_data.xs(inst_str, level=0)
                                    # 获取 close 列（可能是 '$close' 或 'close'）
                                    if '$close' in stock_data.columns:
                                        inst_prices = stock_data['$close']
                                    elif 'close' in stock_data.columns:
                                        inst_prices = stock_data['close']
                                    else:
                                        # 如果没有找到，尝试第一列
                                        inst_prices = stock_data.iloc[:, 0]
                                except (KeyError, IndexError):
                                    true_returns.append(np.nan)
                                    continue
                            else:
                                # 非 MultiIndex 的情况
                                if inst_str in price_data.index:
                                    if '$close' in price_data.columns:
                                        inst_prices = price_data.loc[inst_str, '$close']
                                    elif 'close' in price_data.columns:
                                        inst_prices = price_data.loc[inst_str, 'close']
                                    else:
                                        inst_prices = price_data.loc[inst_str].iloc[0]
                                else:
                                    true_returns.append(np.nan)
                                    continue
                            
                            # 确保 inst_prices 是 Series 或类似的可索引对象
                            if hasattr(inst_prices, '__len__') and len(inst_prices) >= forward_days + 1:
                                # 按时间排序
                                if isinstance(inst_prices.index, pd.DatetimeIndex):
                                    inst_prices = inst_prices.sort_index()
                                
                                start_price = inst_prices.iloc[0]
                                end_price = inst_prices.iloc[forward_days] if len(inst_prices) > forward_days else inst_prices.iloc[-1]
                            ...
                            # 调试信息（可选，可以取消注释查看详细错误）
                            # print(f"    处理 {inst} 时出错: {type(e).__name__}: {e}")
                            true_returns.append(np.nan)
...
if __name__ == '__main__':
    # Windows系统需要freeze_support()来支持multiprocessing
